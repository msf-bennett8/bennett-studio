...
export function GeneralSettings() {
  const { compactSidebar, setCompactSidebar } = useUIPreferencesStore();
      ...
      <div className="card p-6 rounded-xl" style={{ backgroundColor: 'var(--surfaceDefault)', border: '1px solid var(--borderDefault)' }}>
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium" style={{ color: 'var(--textPrimary)' }}>Compact sidebar</p>
              <p className="text-xs" style={{ color: 'var(--textMuted)' }}>Reduces spacing in the navigation sidebar.</p>
            </div>
            <button onClick={() => setCompactSidebar(!compactSidebar)} className="w-12 h-6 rounded-full transition-all relative" style={{ backgroundColor: compactSidebar ? 'var(--accentPrimary)' : 'var(--bgTertiary)' }}>
              <div className="w-5 h-5 rounded-full absolute top-0.5 transition-all" style={{ backgroundColor: 'var(--textInverse)', left: compactSidebar ? '26px' : '2px' }} />
            </button>
          </div>
