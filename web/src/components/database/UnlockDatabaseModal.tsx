    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ backgroundColor: 'var(--bgOverlay)' }}>
      <div className="w-full max-w-md p-6 rounded-2xl shadow-2xl" style={{ backgroundColor: 'var(--bgElevated)', border: '1px solid var(--borderDefault)' }}>
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={onClose} className="p-2 rounded-lg transition-all hover:opacity-80" style={{ backgroundColor: 'var(--bgTertiary)' }}>
          ...
          <div className="mb-4 p-3 rounded-xl" style={{ backgroundColor: 'var(--bgTertiary)', border: '1px solid var(--borderDefault)' }}>
            ...
            <div className="space-y-2">
              {envSuggestions.map((sugg, idx) => (
                <button key={idx} onClick={() => applySuggestion(sugg)} className="w-full text-left p-2 rounded-lg text-xs transition-all hover:opacity-80" style={{ backgroundColor: 'var(--bgSecondary)' }}>
                  <div className="font-medium" style={{ color: 'var(--textPrimary)' }}>{sugg.source}</div>
                  <div className="mt-1" style={{ color: 'var(--textMuted)' }}>User: {sugg.username || '—'} | DB: {sugg.database || '—'}</div>
                </button>
              ))}
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} placeholder="e.g., stacy_bennett_adams" className="input w-full" required disabled={loading} />
          </div>
          <div>
            <label className="block text-sm mb-2 flex items-center gap-2" style={{ color: 'var(--textSecondary)' }}>
            ...
            <div className="relative">
              <input type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter password" className="input w-full pr-10" required disabled={loading} />
              <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-xs" style={{ color: 'var(--textMuted)' }}>
          ...
          <div>
            ...
            <input type="text" value={database} onChange={(e) => setDatabase(e.target.value)} placeholder="e.g., oshocks_local" className="input w-full" required disabled={loading} />
          </div>
          {error && (
          ...
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onClose} disabled={loading} className="btn-secondary flex-1 py-2 rounded-xl disabled:opacity-50">Cancel</button>
            <button type="submit" disabled={!username || !password || loading} className="btn-primary flex-1 py-2 rounded-xl flex items-center justify-center gap-2 disabled:opacity-50">
              {loading ? <div className="w-4 h-4 border-2 border-t-transparent rounded-full animate-spin" /> : <><Unlock size={16} /> Unlock</>}
            </button>
          </div>
        </form>
        <p className="text-xs mt-4 text-center" style={{ color: 'var(--textMuted)' }}>Credentials are stored in memory only and cleared when the app closes.</p>
      </div>
    </div>
  );
}
