...
import { Database, Share2, Terminal, Zap, Shield, Globe } from 'lucide-react';

export function HomePage() {
  const navigate = useNavigate();

  const features = [
    { icon: Database, title: 'Local Databases', description: 'Install PostgreSQL, MySQL, MariaDB, SQLite with one click. Zero configuration required.', color: 'var(--accentPrimary)' },
    { icon: Share2, title: 'Secure Sharing', description: 'Share database access with teammates via secure tunnels. No firewall configuration needed.', color: 'var(--accentSecondary)' },
    { icon: Terminal, title: 'SQL Editor', description: 'Write queries with syntax highlighting, autocomplete, and real-time results.', color: 'var(--accentWarning)' },
    { icon: Zap, title: 'Fast & Lightweight', description: 'Built with Rust and Tauri for maximum performance. Small footprint, big capabilities.', color: 'var(--accentSuccess)' },
    { icon: Shield, title: 'Enterprise Security', description: 'Schema-aware permissions, credential vaulting, audit logging, and end-to-end encryption.', color: 'var(--accentError)' },
    { icon: Globe, title: 'Multi-Client', description: 'Desktop app, web interface, CLI, and VS Code extension — all connected to the same engine.', color: 'var(--accentInfo)' },
  ];
