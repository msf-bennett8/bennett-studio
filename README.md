...
## UPDATED
┌─────────────────────────────────────────────────────────────────────────────┐
│                    EXTERNAL WEBSITE (Vercel/Netlify/etc)                    │
│                                                                             │
...
│                                                                             │
│   // Internally:                                                            │
│   // 1. Parse URL → extract JWT                                             │
│   // 2. Decode JWT → get ICE candidates + host info                         │
│   // 3. Open WebSocket to relay's HTTP port (8443)                        │
│   // 4. Relay forwards through P2P tunnel to your engine                    │
│   // 5. Engine executes query → returns results                             │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
                                      │
                                      ▼ WebSocket/HTTP
┌─────────────────────────────────────────────────────────────────────────────┐
│                         YOUR BENNETT STUDIO (Host)                          │
│                                                                             │
│   ┌─────────────┐     ┌─────────────┐     ┌─────────────┐                 │
│   │   Engine    │◄───►│   Relay     │◄───►│  Browser    │                 │
│   │  (Port 3001)│     │  (Port 8443)│     │  WebSocket  │                 │
│   │             │     │  HTTP API   │     │  Endpoint   │                 │
│   │ • MySQL DB  │     │             │     │             │                 │
│   │ • Queries   │     │ • P2P srvr  │     │ • /ws/share │                 │
│   │ • Schema    │     │ • ICE/Fire  │     │ • /api/share│                 │
│   │ • Export    │     │ • QUIC      │     │   /:code    │                 │
│   └─────────────┘     └─────────────┘     └─────────────┘                 │
│                                                                             │
│   The relay exposes an HTTP/WebSocket API that ANY website can call.        │
│   The relay then proxies those requests through the P2P tunnel.             │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
